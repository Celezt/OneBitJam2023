...
public class SharedSquareGridMeshGenerator : MonoBehaviour
{
    ...
    private struct Generator : IMeshGenerator
    {
        ...
        public void Execute<TStream>(int z, TStream stream) where TStream : struct, IMeshStream
        {
            ...
            {
                float2 xCoordinates = float2(x, x + 1f);
                float2 zCoordinates = float2(z, z + 1f);
